...
const UserSettings = ({ user, onUpdateUser, onNavigate }) => {
  ...
  const validatePassword = () => {
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
      return {
        isValid: false,
        message: "Please fill all password fields or leave them all blank",
      };
    }
  ...
  const handleVerificationChange = (e) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      // 10MB limit for verification docs
      setAlert({
        show: true,
        message: "Document size should be less than 10MB",
        variant: "danger",
      });
      return;
    }
  ...
  const handleUploadVerification = async () => {
    ...
    setVerLoading(true);
    try {
      const resp = await authAPI.uploadVerification(verificationFile);
      ...
      setAlert({
        show: true,
        message:
          resp.message || "Verification document submitted. Awaiting approval.",
        variant: "success",
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Backend unavailable, updating profile locally",
          profileError
        );
        updatedUser = {
          ...user,
          ...formData,
          profileImage:
            previewImage || updatedUser.profileImage || user.profileImage,
          updatedAt: new Date().toISOString(),
        };
        localStorage.setItem(
          "agrohub_current_user",
        ...
        try {
          ...
          setAlert({
            ...
            message:
              ...
              "Failed to change password. Profile changes were saved.",
            variant: "warning",
          });
        }
      } else {
        ...
        <Row>
          <Col lg={8} className="mx-auto">
                      ...
                      <Image
                        src={
                          ...
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(
                            formData.name
                          )}&background=28a745&color=fff&size=120`
                        }
                        alt="Profile"
                        width={120}
                        height={120}
                    ...
                    <Col md={6}>
                      <Form.Group className="mb-3">
                        <Form.Label className="fw-semibold">
                          <i className="fas fa-user me-2 text-success"></i>Full
                          Name
                        </Form.Label>
                        <Form.Control
                          type="text"
                        ...
                        <div className="card-body py-2">
                          <div className="d-flex align-items-center">
                            ...
                            <small className="text-info">
                              <strong>Password Change Mode:</strong> Fill all
                              password fields to update your password
                            </small>
                          </div>
                        </div>
                      </div>
            ...
            <Card className="shadow-sm border-0 mt-4">
              <Card.Header className="bg-white border-bottom-0 py-4">
                <h4 className="mb-0 text-success">
                  <i className="fas fa-file-upload me-2"></i>Verification
                  Documents
                </h4>
              </Card.Header>
              <Card.Body className="p-4">
                <p className="text-muted">
                  Upload an identity or farm registration document so your
                  account can be reviewed and approved.
                </p>
                ...
                <div className="mb-3">
                  <div className="d-flex align-items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        {verLoading ? "Uploading..." : "Submit for Approval"}
                      </Button>
                    </div>
                  </div>
                </div>
                ...
                {user?.verification?.documents &&
                  user.verification.documents.length > 0 && (
                    <div className="mb-2">
                      ...
                      <div>
                        {user.verification.documents.map((d, i) => (
                          <div key={i}>
